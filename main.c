...
int CreateMovieFromFileRow(char *file, long row_id, Movie** movie) {
  ...
  if ((cfPtr = fopen(file, "r")) == NULL) {
    ...
    while (i < row_id) {
// I don't think this is very Time efficient, it's O(n),
// because we loop from the beginning row of the file
// every time we get a (file, rowId) pair!
// What I think would work better is we use another Hashtable
// to store <rowId, row> pairs of each file, just like what
// we've been doning throughout the whole project, so that we make it O(1)
