...
int PutFileInMap(char *filename, DocIdMap map) {
  ...
  HTKeyValue kv;
  HTKeyValue old_kv;
  int ind = NumElemsInHashtable(map);
  kv.key = ind+1;
  kv.value = filename;
  if (kv.key > 64) { kv.key = 64; }
  if (PutInHashtable(map, kv, &old_kv) == 2) {
    if (old_kv.value != kv.value) {
      kv.key++;
      return PutInHashtable(map, kv, &old_kv);
    }
  }
  return 0;
}
...
char *GetFileFromId(DocIdMap docs, int docId) {
  ...
  HTKeyValue result;
  if (LookupInHashtable(docs, docId, &result) == 0) { return result.value;}
  return "FAIL";
}
